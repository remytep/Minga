...
class OrderItemController
{
    ...
    public function __construct(EntityManagerInterface $entityManager, IriConverterInterface $iriConverter, SerializerInterface $serializer, ValidatorInterface $validator, NormalizerInterface $normalizer)
    {
    ...
    #[Route("/order_items", name: "app_order_item", methods: ["POST"])]
    public function createAction(Request $request): Response
    {
        ...
        // normalize the OrderItem entity to an array
        $data = $this->normalizer->normalize($orderItem, 'json', [
            'groups' => ['order_item:read'],
        ]);

        // return a JSON response with the normalized OrderItem entity
        return new Response($this->serializer->serialize($data, 'json'), 201);
    }
}
